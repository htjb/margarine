...
class calculate(object):
    ...
    **Kwargs:**
    ...
    def statistics(self):
        ...
        w = self.theta_weights[mask]/np.sum(self.theta_weights[mask])
        midkl = np.average(mid_logL, weights=w)
        midbd = 2*np.cov(mid_logL, aweights=w)
        de_logL, mask = mask_arr(logprob - base_logprob)
        dekl = np.average(de_logL, weights=w)
        debd = 2*np.cov(de_logL, aweights=w)
        theta_logL, mask = mask_arr(theta_logprob - theta_base_logprob)
        thetakl = np.average(theta_logL, weights=w)
        thetabd = 2*np.cov(theta_logL, aweights=w)
